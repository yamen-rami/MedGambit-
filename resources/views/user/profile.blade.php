                    ...
                    <div class="card mb-6">
                        ...
                        <div class="user-profile-header d-flex flex-column flex-lg-row text-sm-start mb-5 text-center">
                            <div class="mt-n2 mx-sm-0 mx-auto flex-shrink-0">
                                <img
                                    width="140px "
                                    src="{{ asset($user->image ?? '../../assets/img/avatars/1.png') }}"
                                    alt="user image"
                                    class="d-block ms-sm-6 user-profile-img ms-0 h-auto rounded"
                                />
                            </div>
                            <div class="mt-lg-5 mt-3 flex-grow-1">
                                <div class="d-flex align-items-md-end align-items-sm-start align-items-center justify-content-md-between justify-content-start flex-md-row flex-column mx-5 gap-4">
                                    <div class="user-profile-info">
                                        <h4 class="mt-lg-6 mb-2">{{ $user->name }}</h4>
                                        <ul class="list-inline d-flex align-items-center justify-content-sm-start justify-content-center my-2 mb-0 flex-wrap gap-4">
                                            <li class="list-inline-item d-flex align-items-center gap-2">
                                                <i class="icon-base ti tabler-palette icon-lg"></i
                                                ><span
                                                    class="fw-medium"
                                                    >{{ $user->year !== null ? 'Medical Student' : 'Doctor' }}</span>
                                            </li>
                                            <li class="list-inline-item d-flex align-items-center gap-2">
            ...
            <div class="row">
                    ...
                    <div class="card mb-6">
                        <div class="card-body">
                            ...
                            <ul class="list-unstyled mb-0">
                                <li class="d-flex align-items-center mb-4">
                                    <i class="icon-base ti tabler-check icon-lg"></i
                                    ><span class="fw-medium mx-2">Quizzes Completed</span>
                                    <span>{{ $user->attempts->where('status', 'completed')->count() }}</span>
                                </li>
                                <li class="d-flex align-items-center mb-4">
                                    <i class="icon-base ti tabler-layout-grid icon-lg"></i
                                    ><span class="fw-medium mx-2">Quizzez In Completed</span>
                                    <span> {{ $user->attempts->where('status', 'pending')->count() }}</span>
                                </li>
                                <li class="d-flex align-items-center">
                                    <i class="icon-base ti tabler-users icon-lg"></i
                                    ><span class="fw-medium mx-2">Played Quizzez</span>
                ...
                @if (auth()->id() == $user->id)
                    <div class="card col-lg-8 col-md-12 col-sm-12">
                        <table class="table">
                            <div class="col-lg-12">
                                <p class="fw-bold py-6 pb-0">Filtering</p>
                            </div>
                            <thead class="my-5">
                                <tr class="">
                                    <th>Quiz Type</th>
                            ...
                            @foreach ($user->attempts as $attempt)
                                <tbody>
                                    ...
                                    <td class="fw-bold">{{ $attempt->answers->where('is_correct', true)->count() }}</td>
                                    <td class="fw-bold">
