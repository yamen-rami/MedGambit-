...
@enderror
<x-forms.textarea :value="old('options.' . $number . '.explanation')" label="Option {{ $number }} Explantion"
...
<x-forms.textarea :value="old('options.' . $number . '.content')" name="options[{{ $number }}][content]" label="Content"
  type="text"></x-forms.textarea>
...
<div class="d-flex justify-content-between">
  <div class="text-start">
    <button onclick="showPrevious(event, {{ $number }})" class="btn btn-info" {{ $number === "1" ? "disabled" : "" }}
      id="before-{{ $number }}">
      back
    </button>
  </div>
  <div class="text-end">
    <button onclick="showNext(event , {{ $number }})" {{ $number == "5" ? "disabled" : "" }}
      class="{{ $number == "5" ? "btn btn-danger" : "btn btn-primary"  }}" id="next-{{ $number }}">
      Next
    </button>
  </div>
