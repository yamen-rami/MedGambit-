...
new class extends Component {
  ...
  */
  ...
  public $results = [];
  public Quiz $quiz;
  public function mount($quiz, $specialityList, $skillsList, $branchesList)
  {
    $this->quiz = $quiz;
    $this->quizName = $this->quiz->name;
    $this->questionNumber = $this->quiz->questions_number;
    $this->quizLength = $this->quiz->length;
    $this->quizTopic = $this->quiz->topic;
  ...
  public function questions()
  {
    ...
    if (!empty($this->branches)) {
      $query->whereRelation("branches", "BranchOfMedicines.id", $this->branches);
  ...
  {
    $data = $this->validate([
      "questionNumber" => ['nullable', "integer", "min:3", "max:20"],
      "quizName" => ['nullable', "string", "min:3"],
      "quizTopic" => ['nullable', "string", "min:3"],
      "quizDifficulty" => ['nullable', Rule::in(["easy", "medium", "hard", "nerd"])],
      "quizLength" => ['nullable', Rule::in(["short", "medium", "long"])],
    ...
    // faster than foreach 
    $this->quiz->questions()->sync($questions);
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="mb-4 mt-4">
                ...
                <input type="text" name="name" wire:model='quizName' class="form-control"
                  value="{{ $this->quiz->name ?? old("name") }}" id="exampleFormControlInput"
                  placeholder="Enter name" />
              </div>
              @error("name")
                <p class="text-danger">{{ $message }}</p>
              @enderror
              <x-textarea value="{{ $this->quiz->topic }}" wire:model='quizTopic' label="Topic"
                name="quizTopic"></x-textarea>
              <x-forms.input wire:model='questionNumber' value="{{ $this->quiz->questions_number }}"
                label="Questions Number" type="number" max="20" min="2" name="questionNumber"></x-forms.input>
              ...
              <div class="mb-4 mt-4">
                ...
                <select class="form-select" wire:model='quizDifficulty' name="quizDifficulty">
                  <option value="easy" @selected($this->quiz->difficulty === "easy")>Easy</option>
                  <option value="medium" @selected($this->quiz->difficulty === "medium")>Medium</option>
                  <option value="hard" @selected($this->quiz->difficulty === "hard")>Hard</option>
                  <option value="nerd" @selected($this->quiz->difficulty === "nerd")>Nerd</option>
                </select>
              ...
              <div class="mb-4 mt-4">
                ...
                <select class="form-select" wire:model='quizLength' name="quizLength">
                  <option value="short" @selected($this->quiz->length === "short")>Short</option>
                  <option value="medium" @selected($this->quiz->length === "medium")>Medium</option>
                  <option value="long" @selected($this->quiz->length === "long")>Long</option>
                </select>
                @error("quizLength")
                  <p class="text-danger py-1">{{ $message }}</p>
                @enderror
              ...
              <div class="py-2">
                ...
                <div class="d-flex gap-2">
                  <div class="form-check">
                    <label>Easy</label>
                    <input wire:model.live='difficulty' value="easy" class="form-check-input" type="checkbox" />
                  </div>
                  <div class="form-check">
                    <label>Medium</label>
                    <input wire:model.live='difficulty' value="medium" class="form-check-input" type="checkbox" />
                  </div>
                  <div class="form-check">
                    <label>Hard</label>
                    <input wire:model.live='difficulty' value="hard" class="form-check-input" type="checkbox" />
                  </div>
                  <div class="form-check">
                    <label>Nerd</label>
                    <input wire:model.live='difficulty' value="nerd" class="form-check-input" type="checkbox" />
                  </div>
                </div>
              </div>
              <div class="py-2">
                ...
                <div class="d-flex gap-2">
                  <div class="form-check">
                    <label>Short</label>
                    <input wire:model.live='length' value="short" class="form-check-input" type="checkbox" />
                  </div>
                  <div class="form-check">
                    <label>Medium</label>
                    <input wire:model.live='length' value="medium" class="form-check-input" type="checkbox" />
                  </div>
                  <div class="form-check">
                    <label>Long</label>
                    <input wire:model.live='length' value="long" class="form-check-input" type="checkbox" />
                  </div>
                </div>
              </div>
              <div class="py-2">
                ...
                <div class="d-flex gap-2">
                  @foreach ($this->branchesList as $branch)
                    <div class="form-check">
                      ...
                      <input wire:model.live='branches' value="{{ $branch->id }}" class="form-check-input"
                        type="checkbox" />
                    </div>
                  @endforeach
                </div>
                ...
                <div class="d-flex gap-2">
                  @foreach ($this->skillsList as $skill)
                    <div class="form-check">
                      ...
                      <input wire:model.live='skills' value="{{ $skill->id }}" class="form-check-input" type="checkbox" />
                    </div>
                  @endforeach
                </div>
              </div>
                ...
                <div class="d-flex gap-2">
                  @foreach ($this->specialityList as $specality)
                    <div class="form-check">
                      ...
                      <input wire:model.live='specialties' value="{{ $specality->id }}" class="form-check-input"
                        type="checkbox" />
                    </div>
                  @endforeach
                </div>
        ...
        <div class="row py-4   align-items-center">
                ...
                <a class="dropdown-item">
                  <div class="d-flex text-start align-items-center length-parent">
                    ...
                    <div class="length-div">
                      <div>
                        <button type="button" wire:click='filterLenght("short")'
                          class="btn btn-outline-info">Short</button>
                      </div>
                      <div>
                        <button type="button" wire:click='filterLenght("medium")'
                          class="btn btn-outline-info ">Medium</button>
                      </div>
                      <div>
                        <button type="button" wire:click='filterLenght("long")'
                          class="btn btn-outline-info">long</button>
                      </div>
                    </div>
                  </div>
                </a>
                  ...
                  <div class="hover">
                    <div class="d-flex justify-content-start gap-2 align-items-center">
                      ...
                      <div class="difficulty">
                        <div>
                          <button type="button" wire:click='filterDif("easy")'
                            class="btn btn-outline-danger">Easy</button>
                        </div>
                        <div>
                          <button type="button" wire:click='filterDif("medium")'
                            class="btn btn-outline-danger">Medium</button>
                        </div>
                        <div>
                          <button type="button" wire:click='filterDif("hard")'
                            class="btn btn-outline-danger">Hard</button>
                        </div>
                        <div>
                          <button type="button" wire:click='filterDif("nerd")'
                            class="btn btn-outline-danger">Nerd</button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div class="col-lg-4 col-md-4 ">
            <div class="d-flex gap-3">
              {{-- TODO Clear Search --}}
              <input type="text" wire:model.live.debounce='search' name="search" class="form-control ps-5"
